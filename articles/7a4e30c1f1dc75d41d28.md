...
- `AudioSource`の`Play()`、`PlayOneShot()` 等の再生処理を複数個所にハードコードしないこと。
...
sequenceDiagram
    ...
    AddressablesSoundLoader->>AddressablesSoundLoader: SoundId→Address
...
namespace MyGame.Presentation
{
    ...
    {
        // イベント購読
        [Inject] private readonly ISubscriber<PlaySoundRequest> _playSoundRequest = default!;

        [Inject] private readonly ISoundLoader _soundLoader = default!;
...
namespace MyGame.Presentation
{
    public class SampleButton : ...
    {
        ...
        ...
        [Inject] private readonly IPublisher<PlaySoundRequest> _playSoundRequest = default!;
