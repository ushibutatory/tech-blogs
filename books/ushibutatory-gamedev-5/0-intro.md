...
- コードブロックでは`using` 句を全体的に省略しています（行数が長くなる割に、説明したい本質ではないので）。
- ロギングなどの処理も削除しています。
- 似たような記述が続く場合は、「...」のような書き方で省略することがあります。
- 実際のコードをコピペした後に、公開記事用に手直ししているので、整合性が取れなくなっている箇所や説明が欠けてしまっている箇所があるかもしれません（頑張ってチェックしていますが……）。
